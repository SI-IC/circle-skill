...
- `scripts/circle-loop.sh` — детерминированный цикл: выбор фазы (`circle_plan.py next`) → сборка среза
  ...
  пофазный коммит → стоп/продолжение. Рабочая папка — на каждый план своя: `.circle/<имя-плана>/`; цикл
  кладёт в `.circle/` `.gitignore` с `*` (логи несут сырой вывод сессий — вне VCS). Фазу со статусом
  `done` `commit_phase` детерминированно фиксирует (`git add -A && git commit` в репозитории плана,
  `circle: phase <id> — <title>`) — иначе правки копились бы некоммиченными и `git reset --hard` в откате
  поздней фазы снёс бы работу предыдущих. Коммитятся только `done` (blocked/недоделанные откатывают себя
  сами); `add -A` берёт всё дерево репозитория плана (однорепозиторно — мультиworktree у `worktree-skill`).
  Best-effort (сбой коммита не валит цикл); opt-out `CIRCLE_NO_COMMIT=1`.
