...
сессией, % — `null`, если статус-бар не распознан) и `manifest_miss_count` (промахи манифеста).
`context_pct` — независимая от `duration_s` ось: время не отличает пухлую фазу от просто медленной,
а пик контекста отличает. `manifest_miss_count` — сколько раз сессия отчиталась о расхождении
...
- `context_pct=null` при БОЛЬШОМ `duration_s` (живой TUI рисует бар непрерывно) ⇒ извлечение сломано
  ...
  `schema_version`). В такой сессии `run_phase` кладёт в `run-stats/ctx-unparsed.log` строки
  `CIRCLE_CTX_UNPARSED: …` с образцом нераспознанного кадра — по ним чинится регэксп `_CTX_RE`
