...
`subphases_added`, `deps_count`, `autonomy` и `context_pct` (пик потребления контекстного окна
сессией, % — `null`, если статус-бар не распознан). `context_pct` — независимая от `duration_s`
ось: время не отличает пухлую фазу от просто медленной, а пик контекста отличает.
...
- `context_pct=null` при БОЛЬШОМ `duration_s` (живой TUI рисует бар непрерывно) ⇒ извлечение сломано
  (дрейф формата статус-бара), а не «низкая нагрузка». В такой сессии `run_phase` кладёт в `loop.log`
  строки `CIRCLE_CTX_UNPARSED: …` с образцом нераспознанного кадра — по ним чинится регэксп `_CTX_RE`
  (лог локальный, под gitignore; в телеметрию образцы не уходят).
