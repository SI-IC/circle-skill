...
2. Запускает фоновый цикл. На каждой итерации цикл сам выбирает следующую подходящую фазу,
   ...
   Фазу со статусом **`done`** цикл **сам фиксирует в git** — `git add -A && git commit` в репозитории
   плана (сообщение `circle: phase <id> — <title>`). Так работа каждой завершённой фазы попадает в историю
   сразу, а откат `git reset --hard` в заблокированной поздней фазе сбрасывает максимум к коммиту
   предыдущей, а не сносит весь накопленный прогресс. Заблокированные/недоделанные фазы не коммитятся
   (они откатывают свою работу сами). `.circle/` в коммит не попадает (её `*`-gitignore).

   Важно про область коммита: цикл делает `git add -A` в **одном** репозитории — том, что содержит план
   ...
   случайные `.env`/дампы с секретами), попадут в коммит. Коммит локальный (push цикл не делает), но
   гони circle в репозитории, который не жаль так коммитить, — иначе `CIRCLE_NO_COMMIT=1`. План вне
   git-репозитория → коммит пропускается.
...
- `CIRCLE_MAX_SAME_PHASE` — сколько раз подряд одна фаза может выбираться без закрытия до стопа
  «застряла» (по умолч. 3).
- `CIRCLE_NO_COMMIT` — если задан (`1`), цикл не делает пофазных коммитов (по умолч. коммитит —
  см. «Как работает»). Для проектов, где коммитами управляет что-то другое.
